...
###NOVO###
#Ver o campo simulado:
u_ver <- as.vector(inla.spde.make.A(mesh=mesh, loc=grid)%*%as.vector(u))
ggplot() + geom_tile(data=data.frame(x=grid[,1], y=grid[,2], 
                                     u=u_ver), 
                     mapping=aes(x=x,y=y,fill=cut(u,seq(min(u),max(u),length.out=9)))) +
  scale_fill_brewer(type="seq",palette = "YlOrRd") + theme_bw()+
  guides(fill=guide_legend(title="S(x)"))
...
###NOVO###
#DFIndepSample <- DFSim[sample(1:nrow(DFSim), size=150),]
#Anterior fill=ysim
ListSim <- list(DFSim=DFSim, sampData=sampData)

ggplotData <- ggplot() + geom_tile(data=ListSim$DFSim, mapping=aes(x=x,y=y,fill=u_ver)) + 
  scale_fill_viridis_c(option = "B",direction = -1) + 
  geom_point(data=ListSim$sampData,shape = 21,colour = "black", fill = "white", size=1,mapping=aes(x=x,y=y)) +
  theme_bw() 
ggplotData


#Plot com grelha dos eixos
 # ggplotData+theme(
 #   panel.background = element_rect(fill = NA),
 #   panel.grid.minor = element_line(colour = "white"),
 #   panel.ontop = TRUE)

#Figura com a grelha teste MLC
#ggplotData+ geom_vline(xintercept=seq(0,1,1/16), color = "black")+
#geom_hline(yintercept=seq(0,1,1/16), color = "black")
 
 
ggplot() + geom_tile(data=ListSim$DFSim, 
                     mapping=aes(x=x,y=y,fill=cut(ysim,seq(min(ysim),max(ysim),length.out=9)))) +
  scale_fill_brewer(type="seq",palette = "YlOrRd") + 
  geom_point(data=ListSim$sampData, mapping=aes(x=x,y=y)) +
  theme_bw()+
  guides(fill=guide_legend(title="S(x)"))
...
write.table(matrix(RES,ncol=3), file="RES_nocovariate_beta2_100obs_5p_mesh1.txt", append=T,row.names=F, col.names=F)
...
#####Plot da covari·vel#####
DFSim_new <- data.frame(x=grid[,1], y=grid[,2], cov=nndistance)
ListSim <- list(DFSim=DFSim, sampData=sampData, cov=nndistance)

ggplotData <- ggplot(data=ListSim$sampData, mapping=aes(x=x,y=y)) + geom_tile(data=ListSim$DFSim, mapping=aes(x=x,y=y,fill=u_ver)) +
  scale_fill_viridis_c(option = "B",direction = -1) +
  geom_point(aes(size = nndistance),shape = 21,colour = "black", fill = "white") +
  theme_bw()
ggplotData
...
write.table(matrix(RES2,ncol=3), file="RES_covariate_beta2_50obs_5p_mesh1.txt", append=T,row.names=F, col.names=F)
...
#To see results
#n=100
RES_nocovariate_beta2_100obs_5p<-read.table(file="RES_nocovariate_beta2_100obs_5p.txt")
RES_nocovariate_beta2_100obs_5p
aa<-round(RES_nocovariate_beta2_100obs_5p,5)
#library(xtable)
#xtable(aa, type = "latex", file = "filename2.tex")
RES1<-RES_nocovariate_beta2_100obs_5p$V3<0.1
RES1
sum(RES1)


RES_covariate_beta2_100obs_5p<-read.table(file="RES_covariate_beta2_100obs_5p.txt")
RES_covariate_beta2_100obs_5p
bb<-round(RES_covariate_beta2_100obs_5p,5)

#xtable(bb, type = "latex", file = "filename3.tex")
RES2<-RES_covariate_beta2_100obs_5p$V3<0.1
RES2
sum(RES2)

#n=50
RES_nocovariate_beta2_50obs_5p<-read.table(file="RES_nocovariate_beta2_50obs_5p.txt")
RES_nocovariate_beta2_50obs_5p
aa<-round(RES_nocovariate_beta2_50obs_5p,5)
#library(xtable)
#xtable(aa, type = "latex", file = "filename2.tex")
RES1<-RES_nocovariate_beta2_50obs_5p$V3<0.05
RES1
sum(RES1)


RES_covariate_beta2_50obs_5p<-read.table(file="RES_covariate_beta2_50obs_5p.txt")
RES_covariate_beta2_50obs_5p
bb<-round(RES_covariate_beta2_50obs_5p,5)

#xtable(bb, type = "latex", file = "filename3.tex")
RES2<-RES_covariate_beta2_50obs_5p$V3<0.05
RES2
sum(RES2)

#n=250
RES_nocovariate_beta2_250obs_5p<-read.table(file="RES_nocovariate_beta2_250obs_5p.txt")
RES1<-RES_nocovariate_beta2_250obs_5p$V3<0.05
RES1
sum(RES1)


RES_covariate_beta2_250obs_5p<-read.table(file="RES_covariate_beta2_250obs_5p.txt")
RES2<-RES_covariate_beta2_250obs_5p$V3<0.05
RES2
sum(RES2)

#mesh1
#n=100
RES_nocovariate_beta2_100obs_5p<-read.table(file="RES_nocovariate_beta2_100obs_5p_mesh1.txt")
RES_nocovariate_beta2_100obs_5p
aa<-round(RES_nocovariate_beta2_100obs_5p,5)
#library(xtable)
#xtable(aa, type = "latex", file = "filename2.tex")
...
RES_covariate_beta2_100obs_5p<-read.table(file="RES_covariate_beta2_100obs_5p_mesh1.txt")
RES_covariate_beta2_100obs_5p
bb<-round(RES_covariate_beta2_100obs_5p,5)
...
#n=250
RES_nocovariate_beta2_250obs_5p<-read.table(file="RES_nocovariate_beta2_250obs_5p_mesh1.txt")
RES1<-RES_nocovariate_beta2_250obs_5p$V3<0.1
RES1
sum(RES1)


RES_covariate_beta2_250obs_5p<-read.table(file="RES_covariate_beta2_250obs_5p_mesh1.txt")
RES2<-RES_covariate_beta2_250obs_5p$V3<0.1
RES2
sum(RES2)


#n=50
RES_nocovariate_beta2_50obs_5p<-read.table(file="RES_nocovariate_beta2_50obs_5p_mesh1.txt")
RES_nocovariate_beta2_50obs_5p
aa<-round(RES_nocovariate_beta2_50obs_5p,5)
#library(xtable)
#xtable(aa, type = "latex", file = "filename2.tex")
RES1<-RES_nocovariate_beta2_50obs_5p$V3<0.1
RES1
sum(RES1)


RES_covariate_beta2_50obs_5p<-read.table(file="RES_covariate_beta2_50obs_5p_mesh1.txt")
RES_covariate_beta2_50obs_5p
bb<-round(RES_covariate_beta2_50obs_5p,5)

#xtable(bb, type = "latex", file = "filename3.tex")
RES2<-RES_covariate_beta2_50obs_5p$V3<0.1
RES2
sum(RES2)
